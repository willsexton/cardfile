...
for dirent in glob.glob('/Users/dlc32/Data/cards/text/*.txt'):
    file_count += 1
    fh = open(dirent, 'r')
...
from atom_builder import AtomBuilder
builder = AtomBuilder()

pct = (float(matches) / (float(file_count) - 1.0)) * 100.0
print ""
print "%d 'author/recipient' matches found in %d files (%d%%)" % (matches, file_count, pct)
    
#<entry xmlns:duke="http://library.duke.edu/metadata/terms" #xmlns="http://www.w3.org/2005/Atom">
